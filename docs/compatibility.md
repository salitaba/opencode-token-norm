...
| 1.18.30 | Linux | 24 | pass (partial) | 2026-09-11 | Plugin 0.5.1 built fresh from HEAD e302b47 (bundle sha256 `3c5c6b65c455…`; the installed bundle predated `8e3340a` — no `finally` — so it was rebuilt and reinstalled before these runs). Four fresh `opencode run` processes (headless, scratch dir, real `9router/ocg/deepseek-v4-flash`): §1+§8 observe — `announce-threshold at 3 calls` and `budget crossing at 4 calls: tool-calls 4/3` logged, zero `TOKEN NORM -- BUDGET` injected; §8 block — 4th non-cheap call refused with `TOKEN NORM block (mode=block): Tool calls 3 / 3.`, `todowrite` and `handoff` allowed (note written, then the bounded 2s `no session.created` fallback); §6 — subagent `general` refused with `handoff is not available to subagents (you are "general", mode: subagent).`, handoff dir unchanged (84→84); §7 — `opencode serve` + `POST /session/:id/summarize`, hook text incl. `This session has made 1 tool calls` persisted in the rebuilt summary part. Headless caveat: no TUI, so §2–§5 (screen/switch/toast) were not exercised, nor were §8 handoff mode or §9. |
